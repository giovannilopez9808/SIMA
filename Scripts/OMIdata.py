...
def n_month(year,year_i,day):
    month=(datetime.date(year+year_i,1,1)+datetime.timedelta(days=day)).month-1
    return month
...
o3_annual=np.zeros([365,d_year]);o3_mean_month=np.zeros([12,2])
files=listdir(car)
name_year=0
#<------------------------------------Archivo resultante----------------------------------------------->
for file in files:
    ...
    data_o3=list(HD5["/HDFEOS/GRIDS/OMI Column Amount O3/Data Fields/ColumnAmountO3"])
    data,n=0,0
...
for year in range(d_year):
    for day in range(365):
        ...
        if o3>0:
            ...
            o3_mean_month[month,0]+=o3;o3_mean_month[month,1]+=1
#<------------------Calculo del promedio mensual----------------------------------------------------->
print("Calculando promedios")
for month in range(12):
    if o3_mean_month[month,1]!=0:
        o3_mean_month[month,0]=math.ceil(o3_mean_month[month,0]/o3_mean_month[month,1])
