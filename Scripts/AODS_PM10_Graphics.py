...
for station, title, ax, altura in zip(stations, titles, axs, alturas):
    ...
    ax2 = ax.twinx()
    ax.scatter(dates_n, x_PM10, color="#114B5F",
               label="SIMA PM$_{10}$", marker=".", alpha=0.5)
    ax.set_xlim(0, 365*5)
    ax.set_ylim(0, 350)
    ax2.set_ylim(0, 0.1)
    ...
    if ax == axs[1]:
        ...
        ax.set_xticks([])
    ax2.scatter(dates_n, y_SMARTS, color="#DDA15E",
                label="AOD", marker=".", alpha=0.5)
    if ax == axs[0]:
...
# <-----------------------------------------Shared ylabel---------------------------------->
fig.text(0.035, 0.5, 'PM$_{10}$', va='center',
         rotation='vertical', fontsize="large")
fig.text(0.95, 0.5, 'AOD$_{550nm}$', va='center',
         rotation=-90, fontsize="large")
axs[1].set_xlabel("Año", fontsize="large")
plt.subplots_adjust(left=0.125, bottom=0.167, right=0.879,
                    top=0.88, wspace=0.2, hspace=0.188)
