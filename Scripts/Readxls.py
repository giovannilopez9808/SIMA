    ...
    for j in range(sheet.ncols-2):
        ...
        if param==cont:
            ...
            for k in range(int((sheet.nrows-3)/24)):
                ...
                day,month,year=dateformat(day),dateformat(month),(dateformat(year))[2:4]
                file=open(stadir+station+"/Mediciones/"+year+month+day+".txt","w")
                #Ciclo que varia las horas
                for n in range(24):  #Lectura de la medicion
                    med=sheet.cell_value(k*24+n+3,j+2)
                    if med=="": med=0
                    if math.isnan(med)==True: med=0
                    else:med=float(med)
