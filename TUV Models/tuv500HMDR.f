...
! Agregado por Giovanni Gamaliel L�pez Padilla
       character a,stafile*3,type*10
       integer totalarc,arc,i,j,k,n,m,ciclo,si,nsta,sta
       integer,dimension(2):: sii,cont
       real hora,uva,eri,def1,def2
       real,dimension(360,2):: med,mod
       real,dimension(2):: dr,aod,dif


* --- END OF DECLARATIONS ---------------------------------------------
      type="AOD500HMDR"
      open(201,file="../station.txt")
      read(201,*) nsta
      do sta=1,nsta
* re-entry point
...
c      intrct = .FALSE.
     ...
     $     ims,    slabel, imj,    jlabel)
      ...
      ELSE
         iout = 30
      ENDIF
!Se llama al archivo que contiene todos los parametos del dia
          open(100,file="../"//stafile//"/"//type//"/datos500.txt")
!Se llama al archivo que guardara todos los valores de AOD
          open(101,file="../"//stafile//"/"//type//"/AODfound.txt")
          read(100,*) totalarc
!Ciclo que sirve para ir variando los dias
       do arc=1,totalarc
       read(100,*) outfil,tauaer,o3_tc,iyear,imonth,iday
       if(o3_tc.ne.0) then
       open(102,file="../"//stafile//"/"//type//"/Mediciones/v0.0/"
     $      //outfil//"UVAme.txt")
       open(103,file="../"//stafile//"/"//type//"/Mediciones/v0.0/"
     $     //outfil//"Eryme.txt")
       open(105,file="../"//stafile//"/"//type//"/UVA/"
     $     //outfil//"UVAmo.txt")
       open(106,file="../"//stafile//"/"//type//"/Eritemica/"
     $     //outfil//"Erymo.txt")
       do i=1,599
       read(102,*) hora
       read(103,*) hora
       ...
       def2=0
       do i=1,360
       read(102,*) hora,med(i,1)
       read(103,*) hora,med(i,2)
       def1=def1+med(i,1)
       def2=def2+med(i,2)
       end do
       close(102)
       close(103)
       write(*,*) "Calculando el dia ",outfil
!Ciclo que varia las horas
       ...
       tstop=ciclo+1
        k=0
!Ciclo que se detiene cuando se cumple la condicion de la DR
        do while (si.eq.0)
        write(*,*) "Intento N",k
...
* ____ SECTION 9: OUTPUT ______________________________________________
 ...
 30   continue
      CLOSE(iout)
       n=60*(ciclo-10)+1
      open(104,file="../"//outfil//".txt")
      do i=1,132
      read(104,*) a
      end do
      do i=1,60
      read(104,*) a,a,uva,eri
      if(sii(1).eq.0) then
      mod(n,1)=uva
      end if
      if(sii(2).eq.0) then
      ...
      close(104)
!Ciclo que varia para UVA y Eritemica
       do i=1,2
       if (sii(i).eq.0) then
       n=60*(ciclo-10)+1
       dr(i)=0
       cont(i)=0
       dif(i)=0
!Ciclo que varia para los minutos
       do j=1,60
!If que verifica que la medici�n no sea igual a 0
       if((med(n,i).ne.0).and.(sii(i).eq.0)) then
       dr(i)=dr(i)+abs(mod(n,i)-med(n,i))*100/med(n,i)
       dif(i)=dif(i)+(mod(n,i)-med(n,i))*100/med(n,i)
       cont(i)=cont(i)+1
!Cierre del If que verifica que la medici�n no sea igual a 0
       end if
       n=n+1
!Cierre del ciclo que varia los minitos
       end do
       dr(i)=dr(i)/cont(i)
!If que verifica que la condicion de DR
      if (dr(i).le.10) then
      aod(i)=tauaer
      sii(i)=1
!Cierre del if que verifica que la condicion de DR
      end if
!If que verifica que no se encuentre por debajo el modelo
      if (dif(i).lt.0) then
      aod(i)=tauaer
      sii(i)=1
!Cierre del if que verifica que no se encuentre por debajo el modelo
      end if
      end if
      !Cierre del ciclo que varia UVA y Eritemica
      end do
!If que verifica la condicion del ciclo
      if((sii(1).eq.1).and.(sii(2).eq.1)) then
      si=1
      else
      tauaer=tauaer+0.025
!Cierre del if que verifica la condicon del ciclo
      end if
      k=k+1
!Cierre del do con la condicion
      end do
      write(101,*) outfil,int(tstart),aod(1),aod(2),dr(1),dr(2)
!Cierre del ciclo que varia las horas
      end do
      hora=10-0.016666
      do i=1,360
      hora=hora+0.016666
      write(105,*) hora,mod(i,1)
      write(106,*) hora,mod(i,2)
      end do
      close(105)
      close(106)
      end if
!Cierre del ciclo que varia los dias
       end do
      close(100)
      close(101)
