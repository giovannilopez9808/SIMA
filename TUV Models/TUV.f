...
! Agregado por Giovanni Gamaliel L�pez Padilla
      character a,namestation*3
      integer totalarc,arc,i,j,k,n,m,ciclo,si,station,carp
      integer nstation
      integer,dimension(2):: sii,cont
      real,dimension(2)::dif,dr,def,aod
      real hora,uva,eri,op
      real,dimension(480,2):: med,mod
...
c      OPEN(UNIT=kout,FILE='tuvlog',STATUS='UNKNOWN')
      ...
      open(200,file="../2017-2018/stations.txt")
      read(200,*) nstation
      end if
      do station=1,nstation
      if (carp.eq.2) then 
      read(200,*) namestation
      end if
...
c      intrct = .FALSE.}
     ...
     $     ims,    slabel, imj,    jlabel)
      ...
      if (carp.eq.1) then 
      open(100,file="../2016/datos500.txt")
      open(202,file="../2016/aod.txt")
      else
      open(100,file="../2017-2018/"//namestation//"/datos500.txt")
      open(202,file="../2017-2018/"//namestation//"/aod.txt")
      end if
      read(100,*) totalarc
      ...
      tauaer=0.025
      if (carp.eq.1) then
      open(103,file="../2016/Mediciones/v0.0/"//outfil//"Eryme.txt")
      open(104,file="../2016/Mediciones/v0.0/"//outfil//"UVAme.txt")
      else
      open(103,file="../2017-2018/"//namestation//"/Mediciones/v0.0/"
     &//outfil//"Eryme.txt")
      open(104,file="../2017-2018/"//namestation//"/Mediciones/v0.0/"
     &//outfil//"UVAme.txt")
      end if 
      do i=1,480
      read(103,*) hora
...
* ____ SECTION 9: OUTPUT ______________________________________________
 ...
 30   continue
      ...
      close(104)
!Ciclo que varia entre UVA y eritemica
      do i=1,2
      dr(i)=0
      cont(i)=0
      dif(i)=0
...
!If que verifica que la medici�n no sea igual a 0
      ...
      op=(mod(n,i)-med(n,i))*100/med(n,i)
      dr(i)=dr(i)+abs(op)
      dif(i)=dif(i)+op
      cont(i)=cont(i)+1
...
!If que verifica que la condicion de DR
      if((dr(i).le.10).and.(sii(i).eq.0)) then
      sii(i)=1
      aod(i)=tauaer
      else if (dif(i).lt.0) then
      sii(i)=1
      aod(i)=tauaer
...
!Cierre del ciclo que varia UVA Y eritemica
      ...
      if ((sii(1).eq.1).and.(sii(2).eq.1)) then 
      si=1
      write(202,*) outfil,iyear,imonth,iday,aod(1),aod(2)
      end if
