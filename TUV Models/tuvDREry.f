...
! Agregado por Giovanni Gamaliel L�pez Padilla
      ...
      integer totalarc,arc,i,j,k,n,m,ciclo,si,station,carp,cont
      integer nstation
      real hora,uva,eri,def,op,dr,dif
      real,dimension(480):: med,mod
...
c      intrct = .FALSE.}
     ...
     $     ims,    slabel, imj,    jlabel)
      ...
      open(106,file="../2016/resultados/"//outfil//"Erymo.txt")
      else
      open(103,file="../2017-2018/"//namestation//"/Mediciones/v0.0/"
     &//outfil//"Eryme.txt")
      open(106,file="../2017-2018/"//namestation//"/resultados/"
     &//outfil//"Erymo.txt")
      end if 
      do i=1,480
      read(103,*) hora
...
!If que verifica que la medici�n no sea igual a 0
      if((med(n).ne.0).and.(si.eq.0)) then
      op=(mod(n)-med(n))*100/med(n)
      dr=dr+abs(op)
      dif=dif+op
      cont=cont+1
